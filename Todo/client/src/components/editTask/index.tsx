...
export const EditItem = () => {
    ...
    useEffect(() => {
        let getCategories = async () => {
          ...
          let json = await nodeApi.getCategories(auth.user?.id as number)
          setCategories(json)
    ...
    useEffect(() => {
        let request = async () => {
            ...
            let json = await nodeApi.getOne(id as unknown as number)
            setData(json)
    ...
    useEffect(() => {
        ...
        setDate(data?.dueDate as string)
        setCategory(data?.categoryId as number)
    }, [data])
    

    const handleCreateTask = async () => {
        //let json = await API.EditTask(id as unknown as number, title, date, description, category as number)
        let json = await nodeApi.editTask(title, description, date, category, id as unknown as number)
        if(json) {
